...
사용법:
    ...
    python ai-daily.py --no-notes       # octo-notes 저장 건너뛰기
...
DEFAULT_NOTES_DIR = OUTPUT_DIR / "ai-notes"
...
def get_notes_dir(cfg=None):
    """Return the configured notes directory.

    Relative paths in config.ini [저장] notes_dir are resolved from the project root.
    """
    cfg = cfg if cfg is not None else load_config()
    raw = ""
    if cfg.has_section("저장"):
        raw = cfg.get("저장", "notes_dir", fallback="").strip()
    if not raw:
...
# ── octo-notes에 저장 ─────────────────────────────────────
def save_to_notes(filepath, full_date, is_thread=False, notes_dir=None):
    """Configured notes directory에 문서를 복사합니다.

    Notes 저장은 부가 산출물이므로 경로 생성/복사 실패가 전체 실행을
    크래시시키지 않습니다. 실패하면 경고를 남기고 None을 반환합니다.
    """
    import shutil as _shutil

    notes_dir = Path(notes_dir) if notes_dir is not None else get_notes_dir()
    ...
    try:
        ...
        status("WARNING", f"노트 저장 건너뜀: {exc}")
        return None
...
def save_notes_outputs(output_path, thread_path, full_date, no_notes=False, notes_dir=None):
    """Save brief/thread notes unless disabled; return successfully saved paths."""
    if no_notes:
        status("INFO", "노트 저장 건너뜀: --no-notes")
        return []

    saved = []
    for filepath, is_thread in ((output_path, False), (thread_path, True)):
        result = save_to_notes(filepath, full_date, is_thread=is_thread, notes_dir=notes_dir)
...
예시:
  ...
  python ai-daily.py --auto            자동 모드 (cron 용)
    ...
    parser.add_argument("--no-notes", action="store_true", help="노트 저장 안 함")
    ...
    # ── octo-notes 저장 ──
    save_notes_outputs(info["output_path"], thread_path, info["full_date"], no_notes=args.no_notes, notes_dir=get_notes_dir())
