# [38] 모듈형 SSN 온톨로지: W3C·OGC 공동 표준

- 영문 제목: The modular SSN ontology: A joint W3C and OGC standard specifying the semantics of sensors, observations, sampling, and actuation
- 연도: 2018
- 원문 링크: https://doi.org/10.3233/SW-180320
...
W3C(World Wide Web Consortium)와 OGC(Open Geospatial Consortium) SDW(웹상의 공간 데이터) 작업 그룹은 센서, 작동기, 샘플러는 물론 관찰, 작동 및 샘플링 활동을 설명하는 온톨로지 세트를 개발했습니다. 온톨로지는 W3C 권장 사항과 OGC 구현 표준으로 모두 게시되었습니다. 이 세트에는 http://www.w3.org/ns/sosa/에서 사용할 수 있는 SOSA(Sensor, Observation, Sampler 및 Actuator)라는 경량 핵심 모듈과 http://www.w3.org/ns/ssn/에서 사용할 수 있는 SSN(Semantic Sensor Network)이라는 보다 표현력이 풍부한 확장 모듈이 포함되어 있습니다. 이들은 함께 센서 및 작동기 시스템, 관찰, 사용된 절차, 관찰되거나 조치를 취하는 대상 및 속성, 샘플 및 샘플링 프로세스 등을 설명합니다. 온톨로지 세트는 표현성과 폭을 추가하기 위해 SSN 및 기타 모듈에 의해 확장되는 독립형 코어로서 SOSA를 갖춘 모듈식 아키텍처를 채택합니다. SOSA/SSN 온톨로지는 위성 이미지, 대규모 과학 모니터링, 산업 및 가정 인프라, 사회적 감지, 시민 과학, 관찰 기반 온톨로지 엔지니어링, 사물 인터넷을 포함하여 광범위한 애플리케이션 및 사용 사례를 지원할 수 있습니다. 이 문서에서 우리는 온톨로지의 개요를 제공하고 주요 설계 결정의 근거를 논의하며 여기에 제시된 새로운 SSN 온톨로지와 W3C Semantic Sensor Network Incubator 그룹(SSN-XG)에서 개발한 이전 [Web Semantics: Science, Services and Agents on the World Wide Web 17(2012), 25–32] 간의 차이점을 보고합니다. 우리는 사용 사례를 제시하고 다른 온톨로지와의 상호 운용성을 촉진하는 정렬 모듈을 설명합니다.
