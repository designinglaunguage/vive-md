...
2. [전체 시스템 설명](#2-%EC%A0%84%EC%B2%B4-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EC%84%A4%EB%AA%85)
3. [기능 요구사항](#3-%EA%B8%B0%EB%8A%A5-%EC%9A%94%EA%B5%AC%EC%82%AC%ED%95%AD)
4. [비기능 요구사항](#4-%EB%B9%84%EA%B8%B0%EB%8A%A5-%EC%9A%94%EA%B5%AC%EC%82%AC%ED%95%AD)
5. [외부 인터페이스 요구사항](#5-%EC%99%B8%EB%B6%80-%EC%9D%B8%ED%84%B0%ED%8E%98%EC%9D%B4%EC%8A%A4-%EC%9A%94%EA%B5%AC%EC%82%AC%ED%95%AD)
6. [데이터 요구사항](#6-%EB%8D%B0%EC%9D%B4%ED%84%B0-%EC%9A%94%EA%B5%AC%EC%82%AC%ED%95%AD)
7. [제약 조건](#7-%EC%A0%9C%EC%95%BD-%EC%A1%B0%EA%B1%B4)
8. [승인 내역](#8-%EC%8A%B9%EC%9D%B8-%EB%82%B4%EC%97%AD)
...
### 1.3 용어 정의
> 용어 정본은 [용어정의서](../00-기획/용어정의서.md)를 참조한다.
...
| 역할 | 이름 | 소속 | 승인일 | 서명 |
| --- | --- | --- | --- | --- |
| 프로젝트 관리자 (PM) | \[placeholder\] | \[placeholder\] | \[placeholder\] | \_______________\_ |
| 기술 리더 (Tech Lead) | \[placeholder\] | \[placeholder\] | \[placeholder\] | \_______________\_ |
| 비즈니스 오너 | \[placeholder\] | \[placeholder\] | \[placeholder\] | \_______________\_ |
| \[placeholder: 추가 승인자\] | \[placeholder\] | \[placeholder\] | \[placeholder\] | \_______________\_ |
...
### REQ-101: ai-news-collector 노트 저장 경로 설정화 및 안전화

| 항목 | 내용 |
| --- | --- |
| **REQ-ID** | REQ-101 |
| **설명** | ai-news-collector는 노트 저장 경로를 config.ini [저장] 섹션에서 읽고, --no-notes 실행 시 노트 복사를 시도하지 않으며, 노트 경로를 사용할 수 없으면 크래시하지 않고 경고 후 건너뛰어야 한다. |
| **입력** | config.ini [저장] notes_dir 값, --no-notes CLI 플래그 |
| **출력/부작용** | 정상 경로에서는 노트 파일 저장, --no-notes 또는 경로 사용 불가 시 저장 생략 |
| **금지 동작** | 개인 하드코딩 경로에 의존, 노트 경로 생성/쓰기 실패만으로 전체 스크립트 크래시 |
| **검증** | save_to_notes 로컬 유닛테스트, --no-notes 복사 0회 확인, 경로 사용 불가 시 경고와 종료 코드 0 확인 |
